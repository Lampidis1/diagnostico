...
export default async function AdminPage() {
  ...
  const sql = getSql();
  const [rows, users, campaigns] = await Promise.all([
    ...
    session.role === "admin"
      ...
      : Promise.resolve([]),
    session.role === "admin"
      ? sql`SELECT id,subject,template,recipient_count,status,error,created_at FROM email_campaigns ORDER BY created_at DESC LIMIT 10`
      : Promise.resolve([]),
  ]);
  return <Dashboard initialRows={JSON.parse(JSON.stringify(rows))} initialUsers={JSON.parse(JSON.stringify(users))} initialCampaigns={JSON.parse(JSON.stringify(campaigns))} user={session} />;
}
